...
//模擬觸發中獎
setTimeout(winAnimation("恭喜中獎點數"), 1000);
...
function winAnimation(wintxt) {
    ...
    canvasEl.style.cssText = "width: 100vw; height: 100vh; background-color: rgba(0,0,0,0.5); position: absolute; top: 50%; left: 0 ; transform: translateY(-50%)";
    ...
    queue.loadManifest([{ id: "coin1", src: "c1.png" }, { id: "coin2", src: "c2.png" }, { id: "coin3", src: "c3.png" }, { id: "coin4", src: "c4.png" }, { id: "light", src: "LIGHT_3.png" }, { id: "mus", src: "win.mp3" }], true, "scripts/styles/");
    ...
    function init() {
        ...
        var txtArr = wintxt.split("").map(function (val, i) {
            ...
            //文字及外框
            var txt = new createjs.Text(val, "bold " + fontsize + "px  Microsoft JhengHei", "#FFD700");
            ...
            txt_blur.cache(0, 0, txt_blur.getBounds().width, txt_blur.getBounds().height);
        ...
        txtWrap.x = canvas.width / 2 - (txtArr.length * (fontsize + 6) / 2 - (fontsize + 6) / 2); //字的總長以及半個字寬 (因為regX造成起點偏移)
        ...
        if (mobileOverride) {
            ...
            txtWrap.x = canvas.width / 2 - (fontsize + 6) * Math.floor(txtArr.length / 2) / 2; //向後退 長度四分之一的字數+半格
            txtWrap.y = canvas.height * 0.4;
        }

        //獲獎數字
        if (mobileOverride) {
            var unitNum = new createjs.Text('88,800', fontsize * 2 + "px Orelega One", "#FFD700");
        } else {
            var unitNum = new createjs.Text('88,800', fontsize * 2.5 + "px Orelega One", "#FFD700");
        }
        ...
        num_blur.cache(0, 0, num_blur.getBounds().width, num_blur.getBounds().height);
        ...
        rec_mask.graphics.beginFill('#CCCCCC').drawRect(-180, -30, 60, 400).drawRect(-100, -30, 20, 400).endFill();
        rec_mask.skewX = 30;
        ...
        function explode(fw) {
            ...
            hasColor.graphics.f("rgba(255, " + randomC + ", " + randomC + ", 0.8)").dc(0, 0, 2);
        ...
        txtArr.forEach(function (letter, i) {
            if (i == txtArr.length - 1) {
                createjs.Tween.get(letter).wait(100 * i).to({ alpha: 1, scale: 1 }, 800, createjs.Ease.quartIn);

                createjs.Tween.get(numWrap).wait(100 * i + 200).to({ alpha: 1, scale: 1 }, 800, createjs.Ease.quartIn);
                createjs.Tween.get(rec_mask).wait(100 * i + 2000).to({ x: 1200 }, 1300);
                createjs.Tween.get(light).wait(900).to({ alpha: 1 }, 1200);
                createjs.Tween.get(light, { loop: -1 }).to({ rotation: 360 }, 12000);
            } else {
                createjs.Tween.get(letter).wait(120 * i).to({ alpha: 1, scale: 1 }, 800, createjs.Ease.quartIn);
            }
        });
    }
}
